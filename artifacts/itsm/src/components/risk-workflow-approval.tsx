...
export function RiskWorkflowApproval({ row }: { row: Risk }) {
  ...
  function refresh() {
    ...
    qc.invalidateQueries({ queryKey: getGetRiskQueryKey(row.id) });
  }
      ...
      {!activeRun && (
        <div className="rounded-md border border-dashed p-3 flex items-center gap-3">
          <p className="text-sm text-muted-foreground flex-1">
            {row.status === "under_treatment"
              ? row.treatmentDecision
                ? "No active approval workflow on this risk's treatment decision."
                : "Select a treatment decision before starting an approval workflow."
              : "Move risk to Under Treatment before starting an approval workflow."}
          </p>
          {canStart && (
            <Button
              size="sm"
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {decisionFor === "approve"
                ? "Approve this treatment decision"
                : decisionFor === "reject"
                  ? "Reject this treatment decision"
                  : "Defer this treatment decision"}
            </DialogTitle>
          </DialogHeader>
  ...
  return (
    <div className="rounded-md border p-2.5 text-sm space-y-1.5">
      ...
      <p className="text-xs text-muted-foreground">
        ...
        {run.resolvedAt ? new Date(run.resolvedAt).toLocaleDateString() : "—"}
      </p>
      {run.outcomeReason && (
        <p className="text-xs text-muted-foreground">“{run.outcomeReason}”</p>
      )}
...
function WorkflowPicker({
  ...
  const startRun = useStartRiskWorkflowRun();
        ...
        <div className="space-y-2 py-2">
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground">
              No active risk-approval workflows. Create one in Settings →
              Workflows first (module = Risks).
            </p>
          ) : (
            <>
