...
- **Project Management (Phase-driven lifecycle)** — Apr 2026 rework:
    ...
    - **PDF closeout report** (Closed view only): the Actions section on a `closed` project carries an "Export PDF" button next to "Reopen Project" that calls `downloadClosedProjectReport(row)` from `artifacts/itsm/src/components/project-closeout-report.tsx`. The report is generated entirely in the browser via `@react-pdf/renderer` (no server round trip) — a one-page Letter-size document with the &lt;Client Name&gt; wordmark, project name + description, CLOSED + department + priority badges, a Project Details meta grid (owner, assigned team, start, anticipated completion, completed-on, closed-on), the Completion Summary and Key Takeaway / Lessons Learned sections, optional Original Goal + Rationale blocks (rendered only when present on the row), and a two-column Sign-off footer with "Marked Completed by" and "Closed by" name + timestamp. The browser triggers the download as `Project-{slugified-name}-Closeout-{YYYY-MM-DD}.pdf` (date stamp comes from `closedAt`). No PDF library is loaded on the server.
