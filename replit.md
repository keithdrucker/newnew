...
- Department list (`GET /api/departments`) is **always** scoped server-side for non-admins via `visibleDepartmentIds`, regardless of any query parameter — admins see all, agents see only their accessible boards, end_users see their primary department. Prevents cross-department metadata leaks.
- Dashboard endpoints (`/api/dashboard/overview|timeseries|breached`) scope agent queries via `visibleDepartmentIds` with `inArray`, honoring an optional `departmentId` param only when it falls inside the agent's accessible set; an agent on no boards gets an empty result set rather than org-wide stats.
