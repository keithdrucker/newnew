...
- **Project Management (Initiative Pipeline)**:
    - Each initiative is its own top-level **project** under a department. The 7-phase board lives inside the project and tracks the *work steps* needed to deliver that one initiative. Phases: New Suggestions, Future Roadmap, Backlog, Phase 1 - R&D Go/No-Go, Phase 2 - Preparation & Planning, Phase 3 - Implementation, Completed (renamable, e.g. "2026 Completed Initiatives").
    - The old container projects 1 "IT Initiatives" and 43 "IT Operations" were retired in April 2026: each card inside them was promoted to its own top-level project under IT Support Ops (dept 14). Their checklist items became actual cards on the new project's board (done items → "2026 Completed Initiatives" with `completedYear=2026`; not-done → Backlog).
    - The rich Initiative dialog now lives at the **project** level (`ProjectEditorDialog` in `artifacts/itsm/src/components/project-editor-dialog.tsx`). It is opened from "+ New project" on the projects list and from the "Edit details" button on the project board header. It captures: suggestedById, goal, implementation (how), rationale (why), impactedDepartmentIds (jsonb array of dept ids), additionalComments, labels, priority, owner, target date, color.
    - The card editor (`TaskEditorDialog` in `project-board.tsx`) is now a slim **work-step** editor: title, description, phase/bucket, owner, priority, target date, labels, sub-checklist, activity log. The IDEA / PLAN / Impacted-departments / Additional-comments sections are hidden on cards because they live on the parent project.
    - Each checklist item carries its own optional `assigneeId` so different teammates can own different steps. Persisted shape: `{ text, done, assigneeId|null }`. The API hydrates a derived `assigneeName` on read; clients send only `assigneeId`. Server `sanitizeChecklist()` strips `assigneeName` and coerces `assigneeId` on POST/PATCH.
